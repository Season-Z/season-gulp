...
const jsxPath = `${config.paths.react}/*.js`
...
const react = () => {
  return src(jsxPath, { base: config.src })
    .pipe(plugins.sourcemaps.init())
    .pipe(plugins.babel({ presets: [require("@babel/preset-env"), require("@babel/preset-react")] }))
    // .pipe(plugins.react())
    .pipe(plugins.rollup({
      allowRealFiles: true,
      input: './src/index.js',
...
const compile = parallel(style, script, react, page);
const build = series(
  clean,
