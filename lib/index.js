...
const clean = () => {
  return del([config.dist, config.release, config.temp]) // temp为临时目录
}
...
const useref = () => {    // 对打包后的html文件的资源依赖进行整合，将第三方的依赖打包到一个新的文件中。资源的路径也不再引用node_modules里面
