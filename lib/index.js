...
const less = () => {
  return src(lessPath, { base: config.src })
...
const serve = () => {
  ...
  bs.init({
    ...
    server: {
      baseDir: [config.temp, config.dist, config.src, config.public], // 服务会从左到右查找资源，先查dist最后public。如此配置因为图片文字等不需要在开发阶段打包影响效率
      routes: {
...
const compile = parallel(style, less, script, page);
const build = series(
  clean,
