...
const { PROJECT_PATH, SRC_PATH, DIST_PATH, IS_DEV } = require('../config')

module.exports = merge(baseConfig, {
  mode: 'production',
  devtool: IS_DEV ? 'cheap-module-source-map' : false,
  output: {
    path: resolve(PROJECT_PATH, DIST_PATH),
  ...
  plugins: [
     ...
     */
    new PurgeCSSPlugin({
      paths: glob.sync(`${resolve(PROJECT_PATH, SRC_PATH)}/**/*.{tsx,scss,less,css}`, { nodir: true }),
      whitelist: ['html', 'body'],
    }),
  ],
})
